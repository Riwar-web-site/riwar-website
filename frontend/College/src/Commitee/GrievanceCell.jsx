...
function GrievanceCell() {
  ...
  return (
    <div className="grievance-section py-5 bg-light animate__animated animate__fadeIn">
      <Container>
        <h1 className="text-center mb-5 display-5 fw-bold text-primary soft-glow">🛡️ Grievance Redressal Cell</h1>
        <Row className="g-4 justify-content-center">
          ...
          <Col md={6} className="animate__animated animate__fadeInUp hover-effect">
            <Card className="grievance-card p-4 border-0 shadow-lg rounded-4 bg-white">
              <Card.Body>
                <Card.Title className="text-center text-info fw-bold hover-effect">📋 Minutes of Meeting</Card.Title>
                {Object.entries(meetingMinutes).map(([year, notes]) => (
                  <div key={year} className="mb-3">
                    <h6 className="fw-bold text-secondary hover-effect">{year}</h6>
                    <ul className="ps-3">
                      {notes.map((note, idx) => (
                        <li key={idx} className="hover-effect">{note}</li>
                      ))}
                    </ul>
                  </div>
                ))}
